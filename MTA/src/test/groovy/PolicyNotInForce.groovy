...
class PolicyNotInForce extends Specification {
    ...
    // Policy is not in force - says allow and TIA value is True for for Policy is not in force
    def "Scenario 5"() {
    ...
    // Customer can do an MTA (change of vehicle) successfully when business value for eligibility rule
    // Policy is not in force - says allow and TIA value is FALSE for for Policy is not in force
    def "Scenario 6"() {
        given: "User provided mta/check rule using the following schema where Policy is not in force is allow"
        def payload = new JsonBuilder(
                policyNo: "65309936",
